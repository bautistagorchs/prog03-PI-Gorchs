...
class AiringToday extends Component {
  ...
  componentDidMount() {
    const url = `https://api.themoviedb.org/3/tv/airing_today?language=en-US&page=${this.state.page}`;
  ...
  loadMore() {
    ...
    fetch(url, options)
      .then((res) => res.json())
      .then((json) =>
        this.setState({ series: [...this.state.series, ...json.results] }),
      )
      .catch((err) => console.error(err));
  }
  render() {
    const filteredSeries = this.state.series.filter((serie) =>
      serie.name.toLowerCase().includes(this.state.filter.toLowerCase()),
    );
    ...
    return (
      <div>
        <div
          style={{
            display: this.props.displayed === "home" ? "none" : "flex",
            alignItems: "flex-end",
            gap: "0.5rem",
          }}
        >
